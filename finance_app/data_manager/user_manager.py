    ...
    def add_user(self, username, password, full_name="", email="", phone="", date_of_birth="", address="", is_admin=False):
        try:
            ...
            if not is_strong_password(password): # Use imported function
                # Return error dictionary
                logger.warning(f"Attempt to add user {username} with weak password.")
            ...
            if email and not is_valid_email(email): # Use imported function
                logger.warning(f"Attempt to add user {username} with invalid email: {email}")
                return {"status": "error", "message": "Định dạng email không hợp lệ."}
            if email and not self.is_email_unique(email):
                logger.warning(f"Attempt to add user {username} with non-unique email: {email}")
                return {"status": "error", "message": "Địa chỉ email đã được sử dụng."}

            if phone and not is_valid_phone(phone): # Use imported function
                logger.warning(f"Attempt to add user {username} with invalid phone: {phone}")
                return {"status": "error", "message": "Định dạng số điện thoại không hợp lệ. (VD: 10-11 số)"}
            if phone and not self.is_phone_unique(phone):
                logger.warning(f"Attempt to add user {username} with non-unique phone: {phone}")
            ...
            user = {
                ...
                "username": username,
                "password": self.hash_password(password), # hash_password can raise ValueError if processing fails
                "full_name": full_name,
                "email": email,
                "phone": phone,
                "is_admin": is_admin,
            ...
            logger.error(f"ValueError during add_user for {username}: {str(ve)}")
